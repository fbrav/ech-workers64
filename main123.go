...
var (
	listenAddr string
	serverAddr string
	serverIP   string // 支持格式: IP 或 IP:端口
	token      string
	dnsServer  string
	echDomain  string
	xSocks5    string
...
func init() {
	...
	flag.StringVar(&serverAddr, "f", "", "服务端地址 (格式: x.x.workers.dev，默认端口443)")
	flag.StringVar(&serverIP, "ip", "", "指定服务端 IP（格式: IP 或 IP:端口，绕过 DNS 解析）")
...
func queryDoHForProxy(dnsQuery []byte) ([]byte, error) {
	...
	if serverIP != "" {
		transport.DialContext = func(ctx context.Context, network, addr string) (net.Conn, error) {
			// 从 serverIP 解析 IP 和端口
			ip, ipPort, err := parseServerIP(serverIP)
			if err != nil {
				return nil, err
			}
			
			// 如果 serverIP 没有指定端口，使用默认端口
			if ipPort == "" {
				_, defaultPort, err := net.SplitHostPort(addr)
				if err != nil {
					return nil, err
				}
				ipPort = defaultPort
			...
			return dialer.DialContext(ctx, network, net.JoinHostPort(ip, ipPort))
		}
	}
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		...
		if serverIP != "" {
			dialer.NetDial = func(network, address string) (net.Conn, error) {
				...
				// 如果 serverIP 没有指定端口，使用从 serverAddr 解析的端口
				if ipPort == "" {
					ipPort = port
				}
